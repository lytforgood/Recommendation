# -*- coding: utf-8 -*-
"""
Created on Wed Apr 12 20:18:33 2017
...
out_gai = pd.read_csv('./out_cate8_1000.csv')
out_gai = out_gai.loc[:,['user_id']]
def get_chunk_feature(chunk):
    ...
    #print('get y',chunk)
    ###得到y
    #has_action = action.loc[((action['month']==4)&(action['day']>5)&(action['day']<=10))&(action['cate']==8)&(action['type']==4),['user_id']].drop_duplicates()
    y = action.loc[(action['cate']==8),['user_id','sku_id']].drop_duplicates()
    y = pd.merge(out_gai, y, how='left', on=['user_id'])
#    y = action.loc[((action['month']==4)&(action['day']>5)&(action['day']<=10))&(action['cate']==8)&(action['type']==4),['user_id','sku_id']].drop_duplicates()
#    y['label'] = 1
...
#    del has_action
    ...
    action_qian_cate8 = action[(action['cate']==8)]
    ...
    has_action['last_days_gaisku'] = 75 - has_action['day_s']
    ...
    has_action['last_days_cate8'] = 75 - has_action['day_s']
    has_action = has_action.drop(['day_s'],axis=1)
    ...
    has_action = action_qian_cate8.loc[:,['user_id','same_list']]
    has_action = has_action.groupby(by=['user_id'],as_index=False).count()
    ...
    for i in [2,3,4,5]:
        ...
        ###对任意cate是否买过
        has_action = action.loc[(action['type']==i),['user_id']].drop_duplicates()
    ...
    has_action = action.loc[:,['user_id','sku_id','type']]
    has_action['in_gouwuche'] = 0
    has_action.loc[has_action['type']==2,['in_gouwuche']] = 1
    has_action.loc[has_action['type']==3,['in_gouwuche']] = -1
    has_action = has_action.loc[:,['user_id','sku_id','in_gouwuche']].groupby(by=['user_id','sku_id'],as_index=False).sum()
    ...
    print(y.shape)
    y.to_csv('./chunk_feature/f2_cate8_test_feature_chunk'+str(chunk)+'.csv',index=None, encoding='utf-8')

    #has_action = action.loc[((action['month'] == 4)&(action['day']>=6)&(action['day']<11))&(action['cate']==8)&(action['type']==4),['user_id','sku_id']].drop_duplicates()

    product_buy_rate = action.loc[(action['cate']==8)&(action['type']==4),['sku_id']]

    del  action,y
    gc.collect()

    return product_buy_rate


product_buy_rate = []
for i in range(20):
    print('chunk',i)
    buy_rate = get_chunk_feature(i)
...
product_buy_rate.to_csv('./feature_data/test_product_buy_rate.csv',index=False,encoding='utf-8')


del buy_rate, product_buy_rate, all_buy, product, product_buy_rate2
gc.collect()
...
feature = []
for i in range(20):
    y = pd.read_csv('./chunk_feature/f2_cate8_test_feature_chunk'+str(i)+'.csv')
    feature.append(y)
...
last_date = pd.to_datetime('20160416')
...
comment2 = comment.loc[comment['dt'] == '2016-04-15',:]
...
print('to_csv')
feature.to_csv('./feature_data/cate8_test_f2.csv',index=None,encoding='utf-8')
