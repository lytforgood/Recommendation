...
def get_chunk_feature(chunk):
    ...
    y = action.loc[(action['cate']==8),['user_id']].drop_duplicates()
    ...
    has_action = action[(action['cate']==8)]
    #mid_y = y.loc[:,['user_id']]
    ...
    has_action['last_days'] = 75 - has_action['day_s']
    ...
    #print(9, chunk)
    ###近30天是否下单cate8产品
    has_action = action.loc[((action['month']==4)|((action['month']==3)&(action['day']>=17)))&(action['cate']==8)&(action['type']==4),['user_id']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    #has_action = has_action.loc[:,['user_id','sku_id','type']]
    ...
    #print(10, chunk)
    ###是否对cate8下过单
    has_action = action.loc[(action['cate']==8)&(action['type']==4),['user_id']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    #has_action = has_action.loc[:,['user_id','sku_id','type']]
    ...
    #print(11, chunk)
    ###是否下过单
    has_action = action.loc[(action['type']==4),['user_id']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    #has_action = has_action.loc[:,['user_id','sku_id','type']]
    ...
    #print(13, chunk)
    ###交互过的非cate8产品数
    has_action = action.loc[(action['cate']!=8),['user_id','sku_id']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    ...
    #print(14, chunk)
    ###交互过的非cate8产品天数
    has_action = action.loc[(action['cate']!=8),['user_id','day_s']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    ...
    #print(15, chunk)
    ###交互过的非cate8产品次数
    has_action = action.loc[(action['cate']!=8),['user_id','same_list']].drop_duplicates()
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    ...
    has_action = action.loc[(action['cate']!=8),['user_id','sku_id','type']]
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    has_action['jia_gouwuche'] = 0
    has_action.loc[has_action['type']==2,['jia_gouwuche']] = 1
    has_action.loc[has_action['type']==3,['jia_gouwuche']] = -1
    ...
    has_action = action.loc[(action['cate']!=8),['user_id','type']]
    #has_action = pd.merge(has_action, y.loc[:,['user_id']], how='inner',on=['user_id'])
    #has_action = has_action.loc[:,['user_id','type']]
    has_action2 = has_action.groupby(by=['user_id'],as_index=False).count()
...
#    print (y.shape)
    ...
    y.to_csv('./chunk_feature/cate8_test_feature_chunk'+str(chunk)+'.csv',index=None, encoding='utf-8')
    del action, y
    gc.collect()
...
feature = []
for i in range(20):
    y = pd.read_csv('./chunk_feature/cate8_test_feature_chunk'+str(i)+'.csv')
    feature.append(y)
...
feature.to_csv('./feature_data/cate8_test.csv',index=None,encoding='utf-8')